...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Edit Card Type</h2>
        <p class="mt-1 text-sm text-slate-500">Update commission settings for <span class="font-semibold text-indigo-600">{{ $cardType->name }}</span>.</p>
    </div>
</div>

<form action="{{ route('card-types.update', $cardType) }}" method="POST" class="max-w-2xl space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $cardType->name) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="commission_type" id="commission_type"
                        ...
                        <option value="percentage" {{ old('commission_type', $cardType->commission_type) === 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                        <option value="fixed" {{ old('commission_type', $cardType->commission_type) === 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                    </select>
                </div>
                <div>
                    ...
                    <input type="number" name="commission_value" id="commission_value"
                           value="{{ old('commission_value', $cardType->commission_value) }}"
            ...
            <div>
                ...
                <select name="commission_handling" id="commission_handling"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm bg-white focus:border-indigo-400 outline-none">
                    <option value="ignore" {{ old('commission_handling', $cardType->commission_handling) === 'ignore' ? 'selected' : '' }}>Ignore (track only)</option>
                    <option value="auto_write_off" {{ old('commission_handling', $cardType->commission_handling) === 'auto_write_off' ? 'selected' : '' }}>Auto Write-Off (journal entry on sale)</option>
                    <option value="settlement_tracking" {{ old('commission_handling', $cardType->commission_handling) === 'settlement_tracking' ? 'selected' : '' }}>Settlement Tracking (future phase)</option>
                </select>
            ...
            <div>
                ...
                <select name="expense_account_id" id="expense_account_id"
                    ...
                    @foreach($accounts as $account)
                        <option value="{{ $account->id }}" {{ old('expense_account_id', $cardType->expense_account_id) == $account->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="status" id="status" value="1" {{ $cardType->status ? 'checked' : '' }}
