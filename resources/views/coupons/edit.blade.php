...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-[#172033] tracking-tight">Edit Coupon</h1>
            <p class="text-sm text-[#64748B] mt-0.5">Modify details for coupon <span class="font-mono font-bold text-[#F5703E]">{{ $coupon->code }}</span>.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <form action="{{ route('coupons.update', $coupon) }}" method="POST" class="bg-white rounded-xl border border-[#E5E7EB] shadow-sm p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $coupon->code) }}" required
                       class="w-full h-11 px-3.5 bg-white border border-[#E5E7EB] rounded-lg text-sm font-mono uppercase text-[#172033] focus:outline-none focus:border-[#F5703E] focus:ring-1 focus:ring-[#F5703E]">
        ...
        <div class="flex justify-end gap-2.5 pt-4 border-t border-[#E5E7EB]">
            ...
            <button type="submit" class="btn-brand h-10 px-5 rounded-lg text-white text-xs font-medium transition-colors shadow-sm">Update Coupon</button>
        </div>
    </form>
</div>
@endsection
