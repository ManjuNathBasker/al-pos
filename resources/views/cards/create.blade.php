...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-slate-800 mt-2">Create Card Type</h2>
    <p class="mt-1 text-sm text-slate-500">Set up a new card payment type with service charge and bank settlement details.</p>
</div>

<form action="{{ route('cards.store') }}" method="POST" class="space-y-6 max-w-3xl">
    @csrf
            ...
            <div>
                ...
                <select name="card_network" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:bg-white focus:border-indigo-400 outline-none">
                    ...
                    @foreach(['Visa', 'Mastercard', 'RuPay', 'Amex', 'Diners Club', 'JCB', 'Other'] as $network)
                        <option value="{{ $network }}" {{ old('card_network') === $network ? 'selected' : '' }}>{{ $network }}</option>
                    @endforeach
                </select>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <div class="relative">
                    <input type="number" name="service_charge" value="{{ old('service_charge', '0.00') }}" step="0.01" min="0" max="100" required
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="mdr" value="{{ old('mdr', '0.00') }}" step="0.01" min="0" max="100" required
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="processing_fee" value="{{ old('processing_fee', '0.00') }}" step="0.01" min="0" required
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="settlement_account_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:bg-white focus:border-indigo-400 outline-none">
                    ...
                    @foreach($accounts as $account)
                        <option value="{{ $account->id }}" {{ old('settlement_account_id') == $account->id ? 'selected' : '' }}>{{ $account->account_name }}</option>
                    @endforeach
                </select>
                <p class="text-[10px] text-slate-400 mt-1">The bank account where card settlements are received.</p>
            ...
            <div>
                ...
                <input type="number" name="settlement_days" value="{{ old('settlement_days', '1') }}" min="0" required
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:bg-white focus:border-indigo-400 outline-none" />
                <p class="text-[10px] text-slate-400 mt-1">Number of days the bank takes to settle card payments.</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="notes" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:border-indigo-400 outline-none" placeholder="Any additional notes about this card type...">{{ old('notes') }}</textarea>
        </div>
    </div>
